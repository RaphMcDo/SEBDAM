...
#' @param from_sim boolean: true if using simulated data, false if using real data
...
setup_mesh<-function(data=NULL,seed=NULL,nknot=25,model_bound=NULL,
                     crs_input=NULL, from_sim=FALSE,max.edge=c(8,30),cutoff=2, ...) {
  ...
  if (from_sim==FALSE & (is.na(sf::st_crs(data)) | is.na(sf::st_crs(model_bound)))) stop("Data needs a coordinate reference system")
  ...
  } else if (from_sim==TRUE) {
    crs_change<-NA
    utm_data<-data
    utm_bound<-model_bound
  }
