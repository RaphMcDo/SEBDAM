...
fix_param<-function(obj=NULL,pars=NULL){
  ...
  if (!(is.list(pars) & !(is.data.frame(pars)) & any(names(pars) %in% names(obj$env$par)))){
    stop("Either no or wrong parameters provided, or wrong format")
  }

  if (!(all(names(pars) %in% names(obj$env$par)))) {
  ...
  good_names<-names(pars)[which(names(pars) %in% names(obj$env$par))]

  for (params in good_names){
    obj$env$map[params]<-pars[params]
  }
